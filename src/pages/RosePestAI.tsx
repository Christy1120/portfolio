...
const RosePestAI: React.FC = () => {
  ...
  // 核心範疇卡片內容更新為 RosePestAI 相關
  const coreScopes = [
    { id: "01", title: "Literature Review", icon: <Microscope className="w-6 h-6" /> },
    { id: "02", title: "Data Collection", icon: <Cpu className="w-6 h-6" /> },
    { id: "03", title: "Model Training", icon: <Brain className="w-6 h-6" /> },
    { id: "04", title: "Thesis Writing", icon: <Sprout className="w-6 h-6" /> }
  ];

  // 結果展示文字 - 聚焦於 IEEE 發表與最佳簡報獎
  const resultText = "Published the study as first author at IEEE ICCE 2025, where it was recognized with the Best Presentation Award in the Embedded System for IoT Applications session.";
          ...
          {!isAnchorActive ? (
            ...
            >
              ...
              <ul className="flex flex-col gap-4 text-sm font-medium text-gray-400">
                ...
                <li onClick={() => scrollToSection('results')} className="hover:text-pink-400 cursor-pointer transition-colors hover:translate-x-1 transform duration-200">04. Recognition</li>
              </ul>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      >
        <div className="relative z-10 text-center px-6 space-y-6 max-w-4xl mx-auto" ref={addToRefs}>
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tighter text-white drop-shadow-lg leading-tight">
           Deep Learning for Leaf Pest Detection
          </h1>
          <p className="text-xl text-pink-500 font-medium tracking-wide drop-shadow-md max-w-3xl mx-auto">
            Transforming Agricultural Pest Control from Reactive to Proactive
          </p>
      ...
      <main className="max-w-5xl mx-auto px-6 pt-16">
        ...
        <section id="overview" className="mb-32 grid md:grid-cols-2 gap-12 items-center" ref={addToRefs}>
          <div className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              <p className="text-gray-400 leading-relaxed">
                Backed by a research grant from the National Science & Technology Council (NSTC), this initiative aims to bridge the gap between AI technology and real-world agricultural challenges. Rose farmers often face significant losses due to spider mite outbreaks that are difficult to spot manually. As the lead researcher, I spearheaded an end-to-end workflow—from conducting fieldwork for data collection to implementing visual recognition models. 
              </p>
          ...
          <div className="aspect-video bg-[#0a0a0a] rounded-2xl flex items-center justify-center relative overflow-hidden group shadow-2xl border border-white/5">
            ...
            {/* 預留圖片位置 */}
            <img src="/Ai_flower.png" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" /> 
          </div>
        </section>
        ...
        ` 
          ...
          <div className="space-y-6 text-gray-400 leading-relaxed text-lg" ref={addToRefs}>
            ...
            <p><span className="text-pink-500 font-bold mr-2">【Approach】</span>Personally conducted fieldwork at agricultural sites to capture and label over 1,000 high-quality rose leaf images.</p>
            <p><span className="text-pink-500 font-bold mr-2">【Value Delivered】</span>Built a highly structured, reliable dataset that served as the essential foundation for deep learning experiment accuracy.</p>
          </div>

          <div className="space-y-6 text-gray-400 leading-relaxed text-lg pt-8 border-t border-white/5" ref={addToRefs}>
            <div className="flex items-center gap-3">
               <h3 className="text-2xl font-bold text-white tracking-wide">
                 2. Model Prototyping & Validation
               </h3>
            </div>
            <p><span className="text-pink-500 font-bold mr-2">【Approach】</span>Applied and compared various CNN architectures to identify the most effective visual recognition approach.</p>
            <p><span className="text-pink-500 font-bold mr-2">【Value Delivered】</span>Validated recognition accuracy and provided a realistic roadmap for future edge-device deployment.</p>
          </div>

          <div className="space-y-6 text-gray-400 leading-relaxed text-lg pt-8 border-t border-white/5" ref={addToRefs}>
            <div className="flex items-center gap-3">
               <h3 className="text-2xl font-bold text-white tracking-wide">
                 3. User-Centric Problem Framing
               </h3>
            </div>
            <p><span className="text-pink-500 font-bold mr-2">【Approach】</span>Centered the research strategy on real-world agricultural pain points, engineering a solution designed for early-stage intervention based on direct farmer feedback.</p>
            <p><span className="text-pink-500 font-bold mr-2">【Value Delivered】</span>Successfully translated a physical agricultural pain point into a researchable data science problem with tangible real-world value.</p>
          </div>
        </section>
